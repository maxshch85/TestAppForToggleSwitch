...
namespace WpfApp4.Automation.Peers
{
    public class ToggleSwitchAutomationPeer : FrameworkElementAutomationPeer, IToggleProvider
    {
        ...
        public ToggleSwitchAutomationPeer( ToggleSwitchEx owner)
            : base(owner)
        {
        }
        ...
        public ToggleState ToggleState => ConvertToToggleState(((ToggleSwitchEx)this.Owner).IsOn);

        public void Toggle()
        {
            if (this.IsEnabled())
            {
                ((ToggleSwitchEx)this.Owner).AutomationPeerToggle();
            }
        }
    }
}